...
public:
  ...
  virtual std::any visitMulDivMod(PlanParser::MulDivModContext *ctx) override {
    ...
    virtual std::any visitIdentifier(PlanParser::IdentifierContext * ctx)
        override {
...
#define PROCESS_EQALITY(left_type, right_type)                                 \
  if (left_value.type() == typeid(left_type) &&                                \
    ...
    proto::plan::ColumnInfo *getChildColumnInfo(
    ...
    virtual std::any visitReverseRange(PlanParser::ReverseRangeContext * ctx)
        override {
  ...
  if (left_value.type() == typeid(left_type) &&                                \
    ...
    default:                                                                   \
      assert(false);                                                           \
        ...
        PROCESS_ADDSUB(int64_t, int64_t, int64_t,
                       proto::schema::DataType::Int64)
        PROCESS_ADDSUB(int32_t, int32_t, int32_t,
    ...
    virtual std::any visitRelational(PlanParser::RelationalContext * ctx)
        override {
      auto left_expr_with_type =
          std::any_cast<ExprWithDtype>(ctx->expr()[0]->accept(this));
      auto right_expr_with_type =
...
#define PROCESS_RELATIONAL(left_type, right_type)                              \
  \ 
    ...
    virtual std::any visitArrayLength(PlanParser::ArrayLengthContext * ctx)
        override {
    ...
    virtual std::any visitJSONContains(PlanParser::JSONContainsContext * ctx)
        override {
      auto field = std::any_cast<ExprWithDtype>(ctx->expr()[0]->accept(this));
      auto info = field.expr->column_expr().info();
      assert(info.data_type() == proto::schema::DataType::Array ||
  ...
  if (value.type() == typeid(dtype)) {                                         \
    ...
    virtual std::any visitArray(PlanParser::ArrayContext * ctx) override {
      ...
      virtual std::any visitJSONContainsAny(PlanParser::JSONContainsAnyContext *
        ...
        auto value = json_contain_expr->add_elements();
        value->set_allocated_array_val(new proto::plan::Array(
